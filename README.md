...
./start-debug.sh
```
> 调试模式下终端会保持打开,显示所有日志信息。按 `Ctrl+C` 停止服务。
...
- ✨ **现代化界面**：深色主题，玻璃态效果，流畅动画
- 📋 **列表抓取**：输入URL，一键抓取所有商品链接
- 🖼️ **图片下载**：自动读取urls.txt，批量下载高清图片
- ⏹️ **停止控制**：随时停止正在运行的任务，无需确认
- 📝 **实时日志**：查看抓取和下载的详细进度
- 💾 **实时写入**：每页抓取完成后立即写入文件，数据更安全
- 🔄 **自动清理**：每次抓取前自动清空urls.txt，避免重复

## 📦 项目组成

### 0. Web 服务 (`app.py`) ⭐ 推荐
提供美观的 Web 界面，集成所有功能，支持实时日志和任务控制。
...
这会生成 `urls.txt` 文件，每页抓取完成后实时写入：
```text
# Source: https://www.sothebys.com/en/buy/auction/2024/china-5000-years?locale=zh-Hant
# Date: 2025-11-21 08:47:27
# 每页抓取完成后实时写入
# Total Items: 99

https://www.sothebys.com/en/buy/.../item1 # A yellow-ground green and aubergine-enamelled 'dragon' dish...
https://www.sothebys.com/en/buy/.../item2 # Two green-enamelled 'dragon' dishes...
```
...
```
苏富比抓取/
├── A yellow-ground green and aubergine-enamelled.../
│   ├── 001_4096x4096.jpeg
│   ├── 002_4096x4096.jpeg
│   └── ...
├── Two green-enamelled 'dragon' dishes.../
│   ├── 001_4096x4096.jpeg
│   └── ...
└── urls.txt
```
